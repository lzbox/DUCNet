...
from .block import DFL, BNContrastiveHead, ContrastiveHead, Proto, DyDCNv2
from .conv import Conv, DWConv, autopad
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "Detect_TADDH"
...
class Detect_TADDH(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        """用指定数量的类和频道初始化Yolov8检测层."""
        super().__init__()
        self.nc = nc  # 类的数量
        self.nl = len(ch)  # 检测层的数量
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  # 每个锚的产出数量
        self.stride = torch.zeros(self.nl)  # 在构建过程中计算的大步
